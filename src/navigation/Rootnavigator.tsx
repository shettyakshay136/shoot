  ...
  const parseQueryParams = (url: string): { code?: string; state?: string } => {
    try {
      ...
      if (codeMatch) {
        ...
        console.log(
          '✅ [RootNavigator] Extracted code:',
          params.code.substring(0, 30) + '...',
        );
      } else {
      ...
      if (stateMatch) {
        ...
        console.log(
          '✅ [RootNavigator] Extracted state:',
          params.state.substring(0, 30) + '...',
        );
      } else {
  ...
  const linking: LinkingOptions<RootStackParamList> = {
      ...
      try {
        ...
        if (url) {
          ...
          if (url.includes('digilocker') || url.includes('callback')) {
            console.log(
              '✅ [RootNavigator] DigiLocker callback detected in initial URL',
            );
    ...
    subscribe(listener) {
      ...
      const onReceiveURL = ({ url }: { url: string }) => {
        console.log(
          '🔗 [RootNavigator] Deep link received (app running):',
          url,
        );
        if (url.includes('digilocker') || url.includes('callback')) {
      ...
      Linking.getInitialURL().then(url => {
        if (url) {
          ...
          if (url.includes('digilocker') || url.includes('callback')) {
            console.log(
              '✅ [RootNavigator] DigiLocker callback in initial URL',
            );
            const params = parseQueryParams(url);
            console.log('🔗 [RootNavigator] Parsed params:', {
              hasCode: !!params.code,
  ...
  useEffect(() => {
    const handleInitialURL = async () => {
      try {
        ...
        if (
          initialUrl &&
          (initialUrl.includes('digilocker') || initialUrl.includes('callback'))
        ) {
          console.log(
            '✅ [RootNavigator] Found DigiLocker callback in initial URL',
          );
          console.log('🔗 [RootNavigator] Full URL:', initialUrl);
          // The NavigationContainer linking config will handle the navigation
        } else {
          console.log(
            '🔗 [RootNavigator] No DigiLocker callback in initial URL',
          );
        }
      } catch (error) {
        console.error('❌ [RootNavigator] Error getting initial URL:', error);
